...
function UserRegistration() {
  ...
  useEffect(() => {
    ...
    if(userInfo.status === 1){
        ...
        fetch(`${URL}/getPatientMedHistory`,{
...
// updating state variable UserDetails
  ...
  const RegisterMedHist =   async (e) =>{
    ...
    <div className="row mt-3">
        ...
        <div className="row mt-3">
            <label>DID:</label>
            <input type="text" className="form-control" id="did" onChange={RegisterInput} name="did" autoComplete="off" required defaultValue={userInfo.status === 1 ? userInfo.UserDetails.did:''} readOnly={userInfo.status === 1 ? "readOnly":""}/>
        </div>
        <div className="row mt-3">
            <label>Gender:</label>
    ...
    <div className="row mt-3">
                    ...
                    <input type="text" className="form-control" id="digitalAddress" onChange={RegisterInput} placeholder="Enter Patient Digital Address" name="digitalAddress" autoComplete="off" defaultValue={userInfo.status === 1 ? userInfo.UserDetails.digitalAddress:''} readOnly={userInfo.status === 1 ? "readOnly":""} required />
                </div>
    ...
    <div className="row mt-3">
            <button type="submit" onClick={userInfo.status === 1 ? RegisterMedHist : Register} className="btn btn-primary">Next</button>
